...
export default function CloudRegisterPage() {
  ...
  if (done) {
    return (
      <section className="max-w-md mx-auto py-20 px-5" data-testid="cloud-register-done">
        <div className="bg-[#f7f4ef] border border-stone-200 rounded-2xl p-8 text-center">
          <p className="text-[11px] uppercase tracking-[0.28em] text-[#C19A6B] mb-3">ImmobilCloud</p>
          <h2 className="text-2xl font-light tracking-tight mb-2 text-[#0B1E3F]"
  ...
  return (
    <div data-testid="cloud-register-page">
      <CloudPageHero
        eyebrow="ImmobilCloud · Area riservata"
        title={t("cloud.reg_title")}
        subtitle={t("cloud.reg_subtitle")}
        ...
        <form onSubmit={submit} className="bg-white border border-stone-200 rounded-2xl p-7 space-y-5">
          ...
          <Field label={t("cloud.reg_name")}>
            <input data-testid="reg-name" required value={form.name}
              ...
              className="w-full px-3 py-2.5 border border-stone-200 rounded-lg text-sm focus:border-[#0B1E3F] outline-none" />
          </Field>
              ...
              {[
                ...
                <label key={opt.id} data-testid={`reg-intent-${opt.id}`}
                  className={`block p-3 border rounded-xl cursor-pointer transition ${
                    form.intents.includes(opt.id) ? "border-[#0B1E3F] bg-[#f7f4ef]" : "border-stone-200 hover:border-stone-400"
                  }`}>
